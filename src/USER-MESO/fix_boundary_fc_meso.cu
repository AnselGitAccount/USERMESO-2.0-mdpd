...
__global__ void gpu_fix_boundary_fc(
    ...
    int* __restrict mask,
    const int groupbit,
    const int order,        // highest order, for example x^4 --> order=4
    r64* __restrict poly,
    const r64 a0,
    const r64 nx,
    const r64 ny,
    ...
    const int n )
{
    extern __shared__ r64 polycoeffs[];
    for( int p = threadIdx.x; p < order+1; p += blockDim.x )
        polycoeffs[p] = poly[p];
    __syncthreads();
    ...
    for( int i = blockIdx.x * blockDim.x + threadIdx.x; i < n; i += gridDim.x * blockDim.x ) {
        if( mask[i] & groupbit ) {
            ...
            if( d > H - cut ) {
                ...
                r64 force   = a0 * ( polyval( h, order, polycoeffs ) );
                force_x[i] += force * -nx;
                force_y[i] += force * -ny;
                force_z[i] += force * -nz;
            }
...
void MesoFixBoundaryFc::post_force( int vflag )
{
    ...
    gpu_fix_boundary_fc <<< grid_cfg.x, grid_cfg.y, poly.n_byte(), meso_device->stream() >>> (
