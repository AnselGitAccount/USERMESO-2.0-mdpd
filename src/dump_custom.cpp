...
enum{ID,MOL,TYPE,ELEMENT,MASS,
     ...
     IX,IY,IZ,
     VX,VY,VZ,FX,FY,FZ,
     Q,T/*,CONC,CONF*/,MUX,MUY,MUZ,MU,RADIUS,DIAMETER,		// -Ansel
  ...
  if (nthresh) {
    ...
    for (int ithresh = 0; ithresh < nthresh; ithresh++) {
      ...
      if (thresh_array[ithresh] == ID) {
        ...
        ptr = atom->T;
        nstride = 1;
//     } else if (thresh_array[ithresh] == CONC) {			// -Ansel
//        ptr = &atom->CONC[0][0];
//        nstride = 1;
//      } else if (thresh_array[ithresh] == CONF) {			// -Ansel
//        ptr = &atom->CONF[0][0];
//        nstride = 1;
...
int DumpCustom::parse_fields(int narg, char **arg)
{
  ...
  for (int iarg = 5; iarg < narg; iarg++) {
    ...
    if (strcmp(arg[iarg],"id") == 0) {
      ...
      vtype[i] = DOUBLE;
/*    } else if (strcmp(arg[iarg],"CONC") == 0) {					// -Ansel
      n_species = atoi(arg[++iarg]);
      pack_choice[i] = &DumpCustom::pack_CONC;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"CONF") == 0) {			// -Ansel
      n_species = atoi(arg[++iarg]);
      pack_choice[i] = &DumpCustom::pack_CONF;
      vtype[i] = DOUBLE;
...
//    else if (strcmp(arg[1],"CONC") == 0 ) thresh_array[nthresh] = CONC;			// -Ansel
//    else if (strcmp(arg[1],"CONF") == 0 ) thresh_array[nthresh] = CONF;			// -Ansel
...
/* ---------------------------------------------------------------------- */
/*
void DumpCustom::pack_CONC(int n)				// -Ansel
{
...
/* ---------------------------------------------------------------------- */
/*
void DumpCustom::pack_CONF(int n)				// -Ansel
{
  double *CONF = atom->CONF;
